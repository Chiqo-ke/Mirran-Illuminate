...
export function MobileHeroHouse() {
  ...
  const triggerTags = () => {
    ...
    }, 3500); // Disappear after 3.5 seconds of inactivity
  };
      ...
      if (!ticking) {
        window.requestAnimationFrame(() => {
          if (containerRef.current) {
            ...
            // Trigger again if we are actively scrolling while it is in the viewport
  ...
  }, [activeModal]); // Re-bind so triggerTags knows about activeModal state

  const openModal = (id: keyof typeof serviceDetails) => {
    setActiveModal(id);
    setTagsVisible(true); 
    if (hideTimeout.current) clearTimeout(hideTimeout.current); // Stop auto-hide while open
  };

  const closeModal = () => {
    setActiveModal(null);
    triggerTags(); // Restart the visibility cycle once closed
  };

  return (
    <div className="md:hidden flex items-center justify-center mt-20 mb-20 relative" ref={containerRef}>
      {/* Central house icon with soft pulse ring */}
      <div 
        className="group relative w-32 h-32 z-30 flex items-center justify-center" 
        onMouseEnter={() => triggerTags()}
      >
        <div className={cn("absolute inset-0 bg-primary/10 rounded-full transition-opacity duration-500", tagsVisible ? "animate-ping opacity-50" : "opacity-0 group-hover:animate-ping group-hover:opacity-50")}></div>
        <div className={cn("relative z-10 w-24 h-24 bg-background border-2 border-primary/20 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(99,102,241,0.2)] transition-transform duration-500", tagsVisible ? "scale-105" : "scale-100 group-hover:scale-105")}>
        ...
        {/* Top Left - Design */}
        <div className={cn("absolute bottom-full right-1/2 mr-1 pb-4 z-20 transition-all duration-500 ease-out", tagsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto")}>
        ...
        {/* Top Right - Costing */}
        <div className={cn("absolute bottom-full left-1/2 ml-1 pb-4 z-20 transition-all duration-500 ease-out delay-75", tagsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto")}>
        ...
        <div className={cn("absolute top-full right-1/2 mr-1 pt-4 z-20 transition-all duration-500 ease-out delay-150", tagsVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-6 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto")}>
        ...
        <div className={cn("absolute top-full left-1/2 ml-1 pt-4 z-20 transition-all duration-500 ease-out delay-200", tagsVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-6 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto")}>
      ...
      {/* Modal Overlay */}
      <div 
        className={cn("fixed inset-0 z-50 flex items-center justify-center px-4 transition-all duration-300", activeModal ? "opacity-100 visible" : "opacity-0 invisible")}
